...
sub update_change_log ($) {
    ...
    if ($command eq "cvs") {
        ...
        open ERRORS, "/usr/bin/svn update $logname |" or die "The cvs update of ChangeLog failed: $OS_ERROR.\n";
    }
...
if ($command eq "cvs")
...
else
  {
    ...
    open DIFF, "/usr/bin/svn --non-interactive diff --diff-cmd diff -x \"-b\" |" or die "The cvs diff failed: $OS_ERROR.\n";
  }
...
while (<DIFF>)
  {
    ...
    if (defined $file
        ...
        and (/^\d+(,\d+)?[acd](\d+)(,(\d+))?/ or /^Binary files/ or /^Cannot display: file marked as a binary type./) )
      {
